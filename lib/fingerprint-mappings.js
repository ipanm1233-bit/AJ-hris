...
async function fingerprintAttendanceByIds(ids, options = {}) {
  if (!Array.isArray(ids) || !ids.length || ids.length > 30 ||
      ids.some(id => !/^[A-Za-z0-9._-]{1,180}$/.test(String(id || '')))) throw new Error('Pilih 1–30 baris absensi yang valid.');
  const unique = [...new Set(ids.map(String))];
  if (unique.length !== ids.length) throw new Error('Pilihan absensi berulang.');
  const rows = await supabaseRequest('attendance', {
    query: { id: `in.(${unique.map(id => `"${id}"`).join(',')})`, select: '*', limit: 30 }, ...options
  });
  if (!Array.isArray(rows) || rows.length !== unique.length) throw new Error('Sebagian baris absensi tidak ditemukan di Supabase. Muat ulang tabel.');
...
module.exports = { listFingerprintMappings, fingerprintAttendanceById, fingerprintAttendanceByIds, mappingForScan, planManualFingerprintRows, validateManualMapping, saveFingerprintMapping, revokeFingerprintMapping };
