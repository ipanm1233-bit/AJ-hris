...
function fingerprintIdentity(row = {}) {
  ...
  return fingerId && date && branch && String(row.sumber || '').toUpperCase().includes('FINGERPRINT')
...
function hasMappedFingerprintOwner(row, candidates) {
  ...
  return Boolean(identity && candidates.some(candidate =>
...
function dedupeAttendanceRows(rows = []) {
  ...
  rows.forEach(raw => {
    ...
    if (isPendingIdentity(row) && hasMappedFingerprintOwner(row, rows)) return;
