...
function mappedFingerprintOwner(row, candidates) {
  const identity = fingerprintIdentity(row);
  if (!identity) return null;
  const matches = candidates.filter(candidate =>
    !isPendingIdentity(candidate) && fingerprintIdentity(candidate) === identity &&
    sameFingerprintOwner(row, candidate)
  );
  // A machine ID can be reassigned. Never choose between distinct NIKs.
  return new Set(matches.map(candidate => attendanceIdentity(candidate))).size === 1
    ? matches[0] : null;
}
...
function dedupeAttendanceRows(rows = []) {
  ...
  copies.forEach(row => {
    const identity = fingerprintIdentity(row);
    if (!identity || isPendingIdentity(row)) return;
    if (!mappedByFingerprint.has(identity)) mappedByFingerprint.set(identity, []);
    mappedByFingerprint.get(identity).push(row);
  });
  const visible = copies.filter(row => {
    ...
    if (isPendingIdentity(row)) {
      const mapped = mappedFingerprintOwner(row, mappedByFingerprint.get(fingerprintIdentity(row)) || []);
      if (mapped) {
        mapped.scan_masuk ||= row.scan_masuk || null;
  ...
  try {
    ...
    if (body.action === 'attendance_list') {
      ...
      const rows = body.requireSupabase === true
        ? await listAttendance({ fromDate, toDate, branch, nik, limit: safeLimit(body.limit) })
    ...
    if (body.action === 'attendance_reconcile') {
      ...
      const snapshot = await context.db.collection('master_karyawan').get();
      const { planFingerprintReconciliation } = require('./attendance-reconcile.js');
      const plan = planFingerprintReconciliation(rows, snapshot.docs.map(item => ({ ...item.data(), _docId: item.id })));
      ...
      await writeAuditLog(context.db, req, context.user, { action: 'FINGERPRINT_RECONCILE', module: 'absensi', recordId: `${plan.updates.length}_rows`, metadata: { branch, from_date: fromDate, to_date: toDate, unresolved: plan.unresolved } });
      return res.status(200).json({ success: true, mapped: plan.updates.length, unresolved: plan.unresolved });
    }
