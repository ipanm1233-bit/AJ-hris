...
function isFirestoreQuotaError(error) {
  ...
  return code === '8' || code.includes('resource-exhausted') || message.includes('quota exceeded') || message.includes('resource_exhausted');
}

function authProfileFromClaims(uid, decoded = {}) {
  return {
    uid,
    id: decoded.username || uid,
    email: decoded.email || '',
    role: normalizeRole(decoded.role),
    branch: String(decoded.branch || '').trim(),
    division: String(decoded.division || '').trim(),
    ...
    must_change_password: decoded.password_change_required === true
  };
}
...
async function loadAuthProfile(db, uid, decoded = {}) {
  ...
  try {
    ...
    console.warn('[security] Firestore profile quota exhausted; using verified Firebase claims.');
    return authProfileFromClaims(uid, decoded);
  }
  return {
    uid,
    id: profile.id || profile.username || uid,
    ...
    must_change_password: profile.must_change_password === true || decoded.password_change_required === true
  };
}
