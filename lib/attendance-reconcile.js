...
function planFingerprintReconciliation(rows, employees) {
  ...
  for (const row of rows.filter(provisional)) {
    ...
    const fingerId = key(row.fingerprint_user_id || row.fingerprint_no_id);
