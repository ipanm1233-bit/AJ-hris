...
function planFingerprintReconciliation(rows, employees, { verifiedRowsOnly = false } = {}) {
  ...
  for (const row of rows.filter(provisional)) {
    ...
    // Saat kuota Firestore habis, baris Supabase yang telah terpetakan boleh
    // dipakai hanya bila No. ID terpisah dari Emp No., semua bukti hari itu
    // satu NIK, dan nama karyawan maupun nama mesin sama-sama cocok.
    const sameNoId = noId && noId !== fingerId ? rows.filter(item => !provisional(item) &&
      item.tanggal === row.tanggal && key(item.cabang) === branch &&
      key(item.fingerprint_no_id || item.no_id) === noId) : [];
    const verified = sameNoId.length && new Set(sameNoId.map(item => key(item.nik || item.nik_karyawan))).size === 1 &&
      sameNoId.every(item => item.auto_assign === true && machineNameAgrees({ nama: item.nama }, machineName) &&
        machineNameAgrees({ finger_name: item.fingerprint_name || item.nama_finger || item.nama }, machineName))
      ? sameNoId[0] : null;
    const candidate = verifiedRowsOnly ? verified : verified && known.length === 1 && machineNameAgrees(known[0], machineName) ? known[0] :
      explicit.length === 1 ? explicit[0] : explicit.length ? null :
    ...
    if (!nik || !machineName || !machineNameAgrees(candidate, machineName) ||
        ...
        (!(verified && key(verified.nik) === key(nik)) && (historicalFingerprintOwnerConflict(rows.filter(item => key(item.cabang) === branch), fingerId, machineName) ||
        (noId && historicalFingerprintOwnerConflict(rows.filter(item => key(item.cabang) === branch), noId, machineName, 'fingerprint_no_id'))))) {
      unresolved++;
      continue;
    }
