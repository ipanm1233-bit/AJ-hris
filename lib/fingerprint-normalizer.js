...
function scheduleBoundary(schedule = {}) {
  const start = timeToMinutes(schedule.masuk || schedule.jadwal_masuk);
  const end = timeToMinutes(schedule.pulang || schedule.jadwal_keluar);
  if (start === null || end === null || end <= start) return null;
  return { start, end, midpoint: start + ((end - start) / 2) };
}

function computeAttendance(events, existing = {}, minWorkGapMinutes = 120, schedule = {}) {
  ...
  let reviewReason = '';
  let classification = '';
  const boundary = scheduleBoundary(schedule);

  // Beberapa mesin Solution X150 mengirim punchState yang sama untuk semua
  // scan (umumnya 0/check-in), walaupun scan terakhir adalah scan pulang.
  // Jika hanya ada satu jenis arah, gunakan jarak waktu dan jadwal sebagai
  // pengaman. Arah eksplisit tetap dipercaya ketika pasangan IN dan OUT ada.
  if (ins.length && !outs.length) {
    const latestIn = ins[ins.length - 1];
    const loneAfternoonScan = ins.length === 1 && existingIn === null && boundary && latestIn.minutes >= boundary.midpoint;
    if (loneAfternoonScan) {
      scanIn = null;
      scanOut = latestIn;
      classification = 'SINGLE_SCAN_OUT';
      reviewReason = 'Mesin menandai scan sore sebagai check-in; diperkirakan scan pulang. Scan masuk perlu dikoreksi HRD.';
    } else if (scanIn && latestIn.minutes - scanIn.minutes >= minWorkGapMinutes &&
      (!scanOut || latestIn.minutes > scanOut.minutes)) {
      scanOut = latestIn;
      classification = 'SAME_DIRECTION_IN_OUT';
    }
  } else if (outs.length && !ins.length) {
    const earliestOut = outs[0];
    const latestOut = outs[outs.length - 1];
    const loneMorningScan = outs.length === 1 && existingOut === null && boundary && earliestOut.minutes < boundary.midpoint;
    if (loneMorningScan) {
      scanIn = earliestOut;
      scanOut = null;
      classification = 'SINGLE_SCAN_IN';
      reviewReason = 'Mesin menandai scan pagi sebagai check-out; diperkirakan scan masuk. Scan pulang perlu dikoreksi HRD.';
    } else if (!scanIn && latestOut.minutes - earliestOut.minutes >= minWorkGapMinutes) {
      scanIn = earliestOut;
      scanOut = latestOut;
      classification = 'SAME_DIRECTION_IN_OUT';
    }
  ...
  if (unknown.length) {
    // Sebagian mesin tidak mengirim arah IN/OUT. Bila hanya ada satu scan,
    // gunakan jadwal kerja sebagai petunjuk. Scan yang lebih dekat ke jam
    // pulang tidak boleh otomatis dianggap sebagai keterlambatan.
    const onlyUnknown = unknown.length === 1 ? unknown[0] : null;
    const looksLikeCheckout = onlyUnknown && boundary && onlyUnknown.minutes >= boundary.midpoint;
    const existingOnlyContainsSameScan = looksLikeCheckout && scanIn && !scanOut && scanIn.minutes === onlyUnknown.minutes;

    if (existingOnlyContainsSameScan) scanIn = null;
    if (looksLikeCheckout && !scanIn) {
      scanOut = onlyUnknown;
      classification = 'SINGLE_SCAN_OUT';
      reviewReason = 'Hanya ada satu scan sore; diperkirakan scan pulang. Scan masuk perlu dikoreksi HRD.';
    } else if (!scanIn) {
      scanIn = unknown[0];
    }
  ...
  if (!reviewReason && Boolean(scanIn) !== Boolean(scanOut)) {
    classification = scanIn ? 'SINGLE_SCAN_IN' : 'SINGLE_SCAN_OUT';
    reviewReason = scanIn
      ? 'Scan pulang belum terbaca; perlu dikoreksi HRD.'
      : 'Scan masuk belum terbaca; perlu dikoreksi HRD.';
  }
  return {
    scan_masuk: scanIn?.jam || null,
    scan_keluar: scanOut?.jam || null,
