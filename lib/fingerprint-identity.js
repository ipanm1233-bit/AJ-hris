...
function confirmedFingerprintOwnerForDay({ rows = [], employees = [], id, noId, machineName, date, branch }) {
  const userId = normalized(id);
  const machinePin = normalized(noId);
  // Bukti harus datang dari kolom No. ID yang berbeda dari Emp No.; tidak
  // boleh sekadar mencocokkan lagi angka pemilik lama yang bentrok.
  if (!userId || !machinePin || userId === machinePin || !normalized(machineName) || !date) return null;
  const candidates = rows.filter(row =>
    row.tanggal === date && normalized(row.cabang) === normalized(branch) &&
    normalized(row.fingerprint_no_id || row.no_id) === machinePin &&
    !normalized(row.nik || row.nik_karyawan).startsWith('FINGER-') &&
    normalized(row.klasifikasi_scan) !== 'IDENTITY_PENDING'
  );
  if (!candidates.length) return null;
  const owners = new Set(candidates.map(row => normalized(row.nik || row.nik_karyawan)));
  if (owners.size !== 1 || candidates.some(row => !machineNameAgrees({ nama: row.nama }, machineName) ||
      !machineNameAgrees({ finger_name: row.fingerprint_name || row.nama_finger || row.nama }, machineName))) return null;
  const matches = employees.filter(employee => normalized(employee.nik || employee.nik_karyawan) === [...owners][0] &&
    normalized(employee.cabang) === normalized(branch) && machineNameAgrees(employee, machineName));
  return matches.length === 1 ? matches[0] : null;
}

module.exports = { machineNameAgrees, addUniqueIdentifier, historicalFingerprintOwnerConflict, eligibleHistoryForFingerprintFallback, resolveFingerprintEmployee, resolveFingerprintDeviceEmployee, confirmedFingerprintOwnerForDay };
