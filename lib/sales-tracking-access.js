...
const {
  listSalesVisits, getSalesVisitsByIds, upsertSalesVisits, deleteSalesVisits,
...
async function loadVisitsForPatch(context, ids, dependencies = {}) {
  const getPrimary = dependencies.getPrimary || getSalesVisitsByIds;
  const getLegacy = dependencies.getLegacy || (async missingIds => {
    const docs = await Promise.all(missingIds.map(id => context.db.collection('kanal_checkins').doc(id).get()));
    return docs.filter(doc => doc.exists).map(doc => ({ id: doc.id, _docId: doc.id, ...doc.data() }));
  });
  let primary = [];
  if (salesTrackingSupabaseEnabled()) {
    try { primary = await getPrimary(ids); }
    catch (error) {
      if (!fallbackEnabled()) throw error;
      console.warn('[sales-tracking] Supabase visit lookup unavailable:', error.message);
    }
  }
  const byId = new Map(primary.map(row => [clean(row.id), row]));
  const missing = ids.filter(id => !byId.has(id));
  if (missing.length) {
    const legacy = await getLegacy(missing);
    legacy.forEach(row => byId.set(clean(row.id), row));
  }
  return byId;
}
...
async function handleSalesTrackingAccess(req, res, body = {}) {
  ...
  try {
    ...
    if (action === 'sales_track_patch_visits') {
      ...
      const ids = [...new Set(changes.map(change => clean(change.id)))];
      if (ids.some(id => !id)) throw new Error('ID kunjungan wajib diisi.');
      const currentById = await loadVisitsForPatch(context, ids);
      if (ids.some(id => !currentById.has(id))) throw new Error('Kunjungan yang dikoreksi tidak ditemukan. Muat ulang data terlebih dahulu.');
      const rows = changes.map(change => {
        const original = currentById.get(clean(change.id));
        // The stored identity and date decide authorization, not client fields.
        return validateVisit({ ...original, ...change, id: original.id, sales_nik: original.sales_nik,
          tanggal: original.tanggal, cabang: original.cabang });
      });
      const allowed = await applyScope(context, rows);
      if (allowed.length !== rows.length) throw new Error('Ada data kunjungan di luar kewenangan cabang.');
      await writeProviderRows(context, 'visits', rows);
...
module.exports = { handleSalesTrackingAccess, loadProviderRows, loadVisitsForPatch, writeProviderRows, mergeRows, visitIdentity, odometerIdentity, applyScope };
