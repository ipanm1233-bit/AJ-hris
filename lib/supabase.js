...
function getSupabaseConfig(env = process.env) {
  ...
  if (serviceRoleKey.startsWith('sb_publishable_')) {
    throw new Error('Key Supabase server salah: gunakan Secret key (sb_secret_), bukan Publishable key.');
  }
  if (serviceRoleKey.split('.').length === 3) {
    try {
      ...
      if (payload?.role && payload.role !== 'service_role') {
        throw new Error('Key Supabase server salah: gunakan legacy service_role key, bukan anon key.');
      }
    } catch (error) {
      if (/Key Supabase server salah/.test(error.message)) throw error;
    }
  }
...
async function supabaseRequest(path, options = {}) {
  ...
  const response = await (options.fetchImpl || fetch)(`${config.url}/rest/v1/${path}${encodeQuery(options.query)}`, {
    ...
    headers: {
      ...
      // The Supabase gateway derives the elevated database role from apikey.
      ...(config.serviceRoleKey.startsWith('sb_') ? {} : { authorization: `Bearer ${config.serviceRoleKey}` }),
