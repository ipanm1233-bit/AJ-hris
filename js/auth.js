...
export async function mount(container, { session }) {
  ...
  const submenuTabs = container.querySelector("#rmb-submenu-tabs");
  ...
  const btnAddSubmission = container.querySelector("#rmb-btn-add-submission");
  const empFormWrap = container.querySelector("#rmb-employee-form-container");

  if (!isManagement) {
    if (submenuTabs) submenuTabs.classList.add("hidden");
    if (btnAddSubmission) btnAddSubmission.classList.add("hidden");

    // Update Header Text for Employee
    const headerTitle = container.querySelector("h1 span:last-child");
    const headerSub = container.querySelector("h1 + p");
    if (headerTitle) headerTitle.textContent = "Pengajuan Reimbursement";
    if (headerSub) headerSub.textContent = "Formulir pengajuan klaim biaya operasional/medis dan riwayat klaim reimbursement Anda.";

    // Hide filter cabang option (not relevant for regular employee)
    const filterCabang = container.querySelector("#rmb-filter-cabang");
    if (filterCabang && filterCabang.parentElement) filterCabang.parentElement.classList.add("hidden");

    // Change table header title
    const tableTitle = container.querySelector("#rmb-content-pengajuan h3");
    if (tableTitle) tableTitle.textContent = "Riwayat Pengajuan Reimbursement Saya";
  }
  ...
  async function loadMasterTypes() {
    ...
    if (!isManagement && empFormWrap) {
      empFormWrap.classList.remove("hidden");
      renderEmbeddedForm(empFormWrap);
    }
  }

  function renderEmbeddedForm(wrapper) {
    ...
    if (!eligibleTypes.length) {
      wrapper.innerHTML = `
        <div class="p-5 bg-amber-50 border border-amber-200 rounded-2xl text-amber-800 text-xs font-semibold">
          Belum ada jenis reimbursement yang aktif atau dapat diakses oleh akun Anda saat ini.
        </div>
      `;
      return;
    }
        ...
        <div class="flex items-center justify-between border-b border-slate-100 pb-3">
          <div>
            <h3 class="font-bold text-slate-800 text-base flex items-center gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-maroon-700" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>
              <span>Form Pengajuan Reimbursement Baru</span>
            </h3>
            <p class="text-xs text-slate-500 mt-0.5">Isi rincian pengeluaran, pilih jenis klaim, dan unggah foto nota/kuitansi asli.</p>
          </div>
          <span class="px-2.5 py-1 rounded-lg text-xs font-bold bg-maroon-50 text-maroon-800 border border-maroon-100">${escapeHtml(session.nama)} (${escapeHtml(session.posisi || session.role)})</span>
        </div>

        <form id="rmb-embedded-form" class="space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select id="rmb-emb-jenis" required class="w-full px-3.5 py-2.5 text-xs rounded-xl border border-slate-200 focus:border-maroon-400 outline-none bg-white">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="date" id="rmb-emb-tanggal" required value="${new Date().toISOString().slice(0, 10)}" class="w-full px-3.5 py-2.5 text-xs rounded-xl border border-slate-200 focus:border-maroon-400 outline-none">
              </div>
              <div>
                ...
                <input type="number" id="rmb-emb-nominal" required min="1000" step="500" placeholder="Contoh: 150000" class="w-full px-3.5 py-2.5 text-xs rounded-xl border border-slate-200 focus:border-maroon-400 outline-none font-mono">
              </div>
            </div>
          </div>

          <!-- Plafon Information Box -->
          <div id="rmb-emb-plafon-info" class="p-3.5 bg-blue-50/70 border border-blue-100 rounded-xl space-y-1 text-xs text-blue-950"></div>
          ...
          <div>
            ...
            <textarea id="rmb-emb-keterangan" rows="2" required placeholder="Jelaskan secara rinci kebutuhan / transaksi yang dilakukan..." class="w-full px-3.5 py-2 text-xs rounded-xl border border-slate-200 focus:border-maroon-400 outline-none"></textarea>
          </div>

          <!-- Upload Nota -->
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center">
            <div>
              <label class="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1.5">Foto Nota / Kuitansi (Maks 5MB) <span class="text-rose-500">*</span></label>
              <input type="file" id="rmb-emb-file" accept="image/*,.pdf" class="w-full text-xs text-slate-500 file:mr-3 file:py-2 file:px-3 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-maroon-50 file:text-maroon-700 hover:file:bg-maroon-100">
            </div>
            <div id="rmb-emb-preview-wrap" class="hidden flex items-center gap-3">
              <img id="rmb-emb-preview-img" class="h-20 w-auto rounded-xl border border-slate-200 shadow-2xs object-cover">
              <span class="text-[11px] text-emerald-700 font-semibold">✓ Nota Siap Diunggah</span>
            </div>
          </div>

          <div class="pt-2 flex justify-end">
            <button type="submit" id="rmb-emb-btn-submit" class="px-6 py-2.5 bg-maroon-700 hover:bg-maroon-800 text-white font-bold text-xs rounded-xl shadow-sm transition flex items-center gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"/></svg>
              <span>Kirim Pengajuan Reimbursement</span>
            </button>
          </div>
        </form>
      </div>
    ...
    let fileBase64 = "";
    const selectJenis = wrapper.querySelector("#rmb-emb-jenis");
    const boxPlafon = wrapper.querySelector("#rmb-emb-plafon-info");
    const fileInput = wrapper.querySelector("#rmb-emb-file");
    const previewWrap = wrapper.querySelector("#rmb-emb-preview-wrap");
    const previewImg = wrapper.querySelector("#rmb-emb-preview-img");
    const form = wrapper.querySelector("#rmb-embedded-form");
    ...
    form.onsubmit = async (e) => {
      ...
      const typeId = selectJenis.value;
      const typeObj = types.find(t => t.id === typeId);
      const nominal = toNumber(wrapper.querySelector("#rmb-emb-nominal").value);
      const tanggal = wrapper.querySelector("#rmb-emb-tanggal").value;
      const keterangan = wrapper.querySelector("#rmb-emb-keterangan").value.trim();
      ...
      const btnSubmit = wrapper.querySelector("#rmb-emb-btn-submit");
      ...
      try {
        ...
        console.error("Gagal mengirim reimbursement:", err);
