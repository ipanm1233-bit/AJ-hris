...
const CUTI_RULES = {
  ...
  "C-1/2 - Potong Gaji Setengah Hari": { jenis: "Potong Gaji", count: 0.5 },
  "A - Alfa": { jenis: "Tidak Dipotong", count: 0, discipline_impact: true, salary_deduction_review: true },
...
async function processAction(row, action, note, session) {
 ...
 const isPotongGaji = row.is_potong_gaji || row.potong_gaji || (row.detail && (row.detail.is_potong_gaji || row.detail.potong_gaji)) || (row.kategori_cuti || "").includes("Potong Gaji");
      ...
      await fsAdd(COL.MASTER_CUTI, {
        ...
        payroll_consent: Boolean(row.payroll_consent || row.detail?.payroll_consent),
        payroll_consent_at: row.payroll_consent_at || row.detail?.payroll_consent_at || null,
        is_alfa: Boolean(rule.discipline_impact || row.is_alfa || row.detail?.is_alfa),
        alpha_days: rule.discipline_impact ? multiplier : 0,
        discipline_impact: Boolean(rule.discipline_impact || row.discipline_impact || row.detail?.discipline_impact),
        salary_deduction_review_required: Boolean(rule.salary_deduction_review || row.salary_deduction_review_required || row.detail?.salary_deduction_review_required),
