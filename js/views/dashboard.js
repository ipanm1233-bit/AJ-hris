...
async function loadKpiTasks(container, session) {
 ...
 if (!wrap) return;
 try {
 if (!session.nik) { wrap.innerHTML = emptyState("NIK akun belum terhubung ke data karyawan"); return; }
 const q = query(collection(db, COL.TUGAS_KPI_360), where("nik_penilai", "==", session.nik));
 const snap = await getDocs(q);
 ...
 el.onclick = () => { openPenilaianFormFromNotif(pending.find(x => x.id === el.dataset.kpiId), pending, session); };
 });
