...
export async function mount(container, { session }) {
 ...
 ${isSigned ? 'SUDAH TTD' : isPublished ? 'TERBIT' : 'DRAFT'}
 </span>
 </div>
 ...
 <button data-edit-template="${t.id}" class="p-1.5 text-slate-600 hover:bg-slate-200 rounded-lg text-xs" title="Edit Template">Edit</button>
 <button data-del-template="${t.id}" class="p-1.5 text-rose-500 hover:bg-rose-100 rounded-lg text-xs" title="Hapus Template">Hapus Hapus</button>
 </div>
 </div>
 </div>
 `).join("")}
 ...
 ${!p.is_system ? `
 <button data-del-ph="${p.key}" class="p-1 text-rose-500 hover:bg-rose-100 rounded-lg text-xs" title="Hapus Placeholder">Hapus</button>
 ` : ''}
 </div>
 `).join("")}
 </div>`;
...
function openMasterTemplateEditorModal(existingTemplate, onSaveDone) {
 ...
 <button data-tpl-del="${i}" class="p-1 text-rose-500 hover:bg-rose-100 rounded text-xs">Hapus</button>
 </div>
...
function previewDocumentModal(docData) {
 ...
 <p class="text-[9px] text-emerald-600 font-bold">[v] TTD DIGITAL SAH (${fmtDateShort(docData.tanggal_ttd)})</p>
 </div>
 ` : ''}
...
async function openGDocTemplateModal(session) {
 ...
 <button class="btn-remove-custom-gdoc text-rose-600 hover:bg-rose-50 px-2.5 py-1.5 rounded-lg font-bold text-xs transition" title="Hapus Template">Hapus Hapus Template</button>
 </div>
 </div>
