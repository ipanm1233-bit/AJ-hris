...
export async function mount(container, { session } = {}) {
  ...
  async function exportSalesVisitsPdf(recordsOverride = null, pdfVersion = "FULL") {
    ...
    let grandTotalEc = 0;
    const dailyReconciliationDetails = [];
    ...
    salesGroup.forEach((sData, sName) => {
      ...
      dates.forEach(dStr => {
        ...
        const hasKmStart = odometer.km_awal !== undefined && odometer.km_awal !== null && odometer.km_awal !== "";
        const hasKmEnd = odometer.km_akhir !== undefined && odometer.km_akhir !== null && odometer.km_akhir !== "";
        const kmStart = hasKmStart ? Number(odometer.km_awal) : null;
        const kmEnd = hasKmEnd ? Number(odometer.km_akhir) : null;
        const odometerDistance = odometer.jarak_odometer !== undefined && odometer.jarak_odometer !== null
          ? Number(odometer.jarak_odometer)
          : Math.max(0, Number(odometer.km_akhir || 0) - Number(odometer.km_awal || 0));
        const uniqueStoreCount = new Set(visits.map(item => cleanStoreName(item.toko_outlet || ""))).size;
        const reconciliation = buildDailyRouteReconciliation({
          gpsKm: eff.totalKm,
          odometerKm: odometerDistance,
          storeCount: uniqueStoreCount
        });
        dailyReconciliationDetails.push({
          tanggal: dStr,
          nama: sName,
          nik: sData.nik,
    ...
    const sortedDailyReconciliation = [...dailyReconciliationDetails]
      .sort((a, b) => b.tanggal.localeCompare(a.tanggal) || a.nama.localeCompare(b.nama, "id"));
    const dailyReconciliationRowsHtml = sortedDailyReconciliation.map((row, idx) => {
      const hasOdometerReading = row.kmStart !== null && row.kmEnd !== null;
      const differenceColor = Math.abs(row.differenceKm) > 5 ? "#b91c1c" : "#475569";
      const status = !hasOdometerReading
        ? `<span style="color:#b45309;font-weight:bold;">Odometer belum lengkap</span>`
        : Math.abs(row.differenceKm) > 5
          ? `<span style="color:#b91c1c;font-weight:bold;">Perlu pemeriksaan</span>`
          : `<span style="color:#15803d;font-weight:bold;">Sesuai toleransi</span>`;
      return `
        <tr style="border-bottom:1px solid #e2e8f0; font-size:9px; page-break-inside:avoid;">
          <td style="padding:5px; text-align:center;">${idx + 1}</td>
          <td style="padding:5px; white-space:nowrap; font-weight:bold;">${escapeHtml(row.tanggal)}</td>
          <td style="padding:5px;"><b>${escapeHtml(row.nama)}</b><div style="font-size:8px;color:#64748b;">${escapeHtml(row.nik)}</div></td>
          <td style="padding:5px; text-align:center;">${row.visitCount} visit<br><span style="font-size:8px;color:#64748b;">${row.storeCount} toko</span></td>
          <td style="padding:5px; text-align:right;">${row.kmStart === null ? "-" : `${row.kmStart.toFixed(1)} KM`}</td>
          <td style="padding:5px; text-align:right;">${row.kmEnd === null ? "-" : `${row.kmEnd.toFixed(1)} KM`}</td>
          <td style="padding:5px; text-align:right; font-weight:bold; color:#0f172a;">${row.odometerKm.toFixed(1)} KM</td>
          <td style="padding:5px; text-align:right; font-weight:bold; color:#4338ca;">${row.gpsKm.toFixed(1)} KM</td>
          <td style="padding:5px; text-align:right; font-weight:bold; color:${differenceColor};">${row.differenceKm > 0 ? "+" : ""}${row.differenceKm.toFixed(1)} KM</td>
          <td style="padding:5px; text-align:right; font-weight:bold; color:#15803d;">Rp${row.claimAmount.toLocaleString("id-ID")}</td>
          <td style="padding:5px; text-align:center; font-size:8px;">${status}</td>
        </tr>
      `;
    }).join("");
    ...
    if (pdfVersion === "FULL") {
      salesmanSummaries.forEach(s => {
        ...
        dates.forEach(dStr => {
          ...
          const dailyKmStart = dailyOdometer.km_awal !== undefined && dailyOdometer.km_awal !== null && dailyOdometer.km_awal !== "" ? Number(dailyOdometer.km_awal) : null;
          const dailyKmEnd = dailyOdometer.km_akhir !== undefined && dailyOdometer.km_akhir !== null && dailyOdometer.km_akhir !== "" ? Number(dailyOdometer.km_akhir) : null;
          const dailyRecon = buildDailyRouteReconciliation({
            gpsKm: effGps.totalKm,
            odometerKm: dailyOdometer.jarak_odometer !== undefined && dailyOdometer.jarak_odometer !== null
              ? Number(dailyOdometer.jarak_odometer)
              : Math.max(0, Number(dailyOdometer.km_akhir || 0) - Number(dailyOdometer.km_awal || 0)),
          ...
          detailedHtml += `
            <div style="margin-top: 8px; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; background-color: #ffffff;">
              ...
              <div style="padding:6px 10px; background:#fffbeb; border-bottom:1px solid #fde68a; font-size:9px; font-weight:bold;">
                <div style="display:flex; gap:6px;">
                  <span style="flex:1;">Kunjungan: ${visits.length} visit / ${dailyRecon.storeCount} toko</span>
                  <span style="flex:1;">KM Awal: ${dailyKmStart === null ? '-' : `${dailyKmStart.toFixed(1)} KM`}</span>
                  <span style="flex:1;">KM Akhir: ${dailyKmEnd === null ? '-' : `${dailyKmEnd.toFixed(1)} KM`}</span>
                  <span style="flex:1; color:#0f172a;">Total Odometer: ${dailyRecon.odometerKm.toFixed(1)} KM</span>
                </div>
                <div style="display:flex; gap:6px; margin-top:4px; padding-top:4px; border-top:1px dashed #fcd34d;">
                  <span style="flex:1; color:#4338ca;">Total Tracking: ${dailyRecon.gpsKm.toFixed(1)} KM</span>
                  <span style="flex:1; color:${Math.abs(dailyRecon.differenceKm) > 5 ? '#b91c1c' : '#475569'};">Selisih Odo - Tracking: ${dailyRecon.differenceKm > 0 ? '+' : ''}${dailyRecon.differenceKm.toFixed(1)} KM</span>
                  <span style="flex:1; color:#15803d;">Klaim Harian: Rp${dailyRecon.claimAmount.toLocaleString('id-ID')}</span>
                  <span style="flex:1; color:#64748b;">Rumus: ${dailyRecon.odometerKm.toFixed(1)} - ${dailyRecon.gpsKm.toFixed(1)} KM</span>
                </div>
              </div>
    ...
    const fullPdfHtml = `
      <div style="font-family: Arial, Helvetica, sans-serif; color: #1e293b; padding: 16px; background-color: #ffffff; max-width: 800px; margin: 0 auto;">
        ...
        <!-- BAGIAN 2: REKONSILIASI HARIAN ODOMETER VS TRACKING -->
        <div style="margin-bottom:14px;">
          <h4 style="font-size:10px; font-weight:bold; color:#7a1f2b; margin:0 0 5px 0; border-bottom:1.5px solid #7a1f2b; padding-bottom:2px;">2. REKONSILIASI HARIAN ODOMETER VS PERHITUNGAN TRACKING</h4>
          <table style="width:100%; border-collapse:collapse; text-align:left; border:1px solid #cbd5e1;">
            <thead>
              <tr style="background:#e2e8f0; color:#1e293b; font-size:8px; text-transform:uppercase;">
                <th style="padding:5px;text-align:center;">#</th>
                <th style="padding:5px;">Tanggal</th>
                <th style="padding:5px;">Sales</th>
                <th style="padding:5px;text-align:center;">Visit/Toko</th>
                <th style="padding:5px;text-align:right;">KM Awal</th>
                <th style="padding:5px;text-align:right;">KM Akhir</th>
                <th style="padding:5px;text-align:right;">Total Odo</th>
                <th style="padding:5px;text-align:right;">Total Tracking</th>
                <th style="padding:5px;text-align:right;">Selisih</th>
                <th style="padding:5px;text-align:right;">Klaim</th>
                <th style="padding:5px;text-align:center;">Status</th>
              </tr>
            </thead>
            <tbody>${dailyReconciliationRowsHtml}</tbody>
            <tfoot>
              <tr style="background:#f8fafc; border-top:1.5px solid #94a3b8; font-size:9px; font-weight:bold;">
                <td colspan="6" style="padding:6px; text-align:right;">TOTAL PERIODE</td>
                <td style="padding:6px; text-align:right;">${grandTotalOdometer.toFixed(1)} KM</td>
                <td style="padding:6px; text-align:right; color:#4338ca;">${grandTotalKm.toFixed(1)} KM</td>
                <td style="padding:6px; text-align:right; color:${Math.abs(grandTotalOdometer - grandTotalKm) > 5 ? '#b91c1c' : '#475569'};">${grandTotalOdometer - grandTotalKm > 0 ? '+' : ''}${(grandTotalOdometer - grandTotalKm).toFixed(1)} KM</td>
                <td style="padding:6px; text-align:right; color:#15803d;">Rp${grandTotalClaim.toLocaleString('id-ID')}</td>
                <td style="padding:6px; text-align:center;">${sortedDailyReconciliation.length} hari-sales</td>
              </tr>
            </tfoot>
          </table>
          <p style="font-size:8px; color:#64748b; margin:4px 0 0;">Selisih = total odometer harian dikurangi total jarak tracking/Google Maps. Klaim dihitung dari jarak odometer sesuai ketentuan perusahaan.</p>
        </div>

        ${pdfVersion === "FULL" ? `
        <!-- BAGIAN 3: DETAIL HARIAN PER SALES & FOTO BUKTI FIELD -->
        <div style="margin-bottom: 14px;">
          <h4 style="font-size: 10px; font-weight: bold; color: #7a1f2b; margin: 0 0 5px 0; border-bottom: 1.5px solid #7a1f2b; padding-bottom: 2px;">3. DETAIL RINCIAN KUNJUNGAN HARI DEMI HARI & FOTO BUKTI FIELD</h4>
          ${detailedHtml}
        </div>
        ` : ''}
    ...
    try {
      ...
      // Landscape menjaga tabel rekonsiliasi harian dan detail rute tetap
      // terbaca tanpa mengecilkan 11 kolom menjadi terlalu rapat.
      await downloadHtmlAsPdf(fullPdfHtml, `Laporan_Rute_Sales_${fileNameSuffix}_${todayStr}.pdf`, "landscape");
