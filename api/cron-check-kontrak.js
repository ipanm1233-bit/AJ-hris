...
module.exports = async function handler(req, res) {
  ...
  } // <--- KURUNG INI HILANG DI KODE ANDA
  ...
  const tokenMap = {};
  usersSnap.forEach(d => { 
      if (d.data().role === 'HRD' || d.data().role === 'SUPERADMIN') { 
          if (d.data().fcm_token) tokenMap[d.id] = d.data().fcm_token; 
      } 
  });
  ...
  if (tokens.length && !kontrakSnap.empty) {
    await admin.messaging().sendEachForMulticast({
      notification: { title: "⚠️ Kontrak Segera Habis", body: `${kontrakSnap.size} kontrak karyawan perlu ditinjau.` },
      tokens
    });
  } // <--- KURUNG INI HILANG DI KODE ANDA
