const { enforceRateLimit, requireFirebaseAuth, writeAuditLog, isSameBranch } = require('../lib/security.js');
...
module.exports = async function handler(req, res) {
  ...
  if (!context) return;
  if (!enforceRateLimit(req, res, { namespace: 'training', key: context.user.uid, limit: 600, windowMs: 60 * 60_000 })) return;
  const body = req.body || {};
  try {
    if (body.action === 'list') {
      return res.status(200).json({ success: true, data: await listAll(context.db, context.user) });
    }
    if (body.action === 'bulk_assignments') {
      ...
      return res.status(200).json({ success: true, count: body.rows.length });
    }
    ...
    return res.status(400).json({ success: false, error: error.message || 'Operasi pelatihan gagal.' });
  }
};

module.exports._test = { readable, owns, onlyKeys, sanitizePayload };
